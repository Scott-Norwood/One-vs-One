...
{
    #region System_Recoil_Core Class
    /// <summary>
    ...
    public class System_Recoil_Core : MonoBehaviour
    {
        ...
        public void Recoil()
        {
            ...
            // Calculate the number of degrees we can recoil backwards before hitting angle limit.
            float spare = (360 + -maxAngle);

            // Check if we have enough room to recoil backwards.
            if (transform.eulerAngles.x - _climbAmount < spare && transform.eulerAngles.x > -minAngle + 30)
            {
                // We have to clamp the recoil.
            }
            else
            {
